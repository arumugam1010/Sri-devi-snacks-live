        ...
        try {
            ...
            if (!$migrated) {
                ...
                try {
                    ...
                    // Check and add image, role, blood_group, and address columns to employees table
                    $checkCol = $pdo->query("SHOW COLUMNS FROM employees LIKE 'image'");
                    if (!$checkCol->fetch()) {
                        $pdo->exec("ALTER TABLE employees ADD COLUMN image LONGTEXT NULL");
                    }
                    $checkCol = $pdo->query("SHOW COLUMNS FROM employees LIKE 'role'");
                    if (!$checkCol->fetch()) {
                        $pdo->exec("ALTER TABLE employees ADD COLUMN role VARCHAR(100) NULL");
                    }
                    $checkCol = $pdo->query("SHOW COLUMNS FROM employees LIKE 'blood_group'");
                    if (!$checkCol->fetch()) {
                        $pdo->exec("ALTER TABLE employees ADD COLUMN blood_group VARCHAR(10) NULL");
                    }
                    $checkCol = $pdo->query("SHOW COLUMNS FROM employees LIKE 'address'");
                    if (!$checkCol->fetch()) {
                        $pdo->exec("ALTER TABLE employees ADD COLUMN address TEXT NULL");
                    }
                    ...
                    // Backfill existing employees missing employee_code
                    try {
                        $missingEmps = $pdo->query("SELECT id, joining_date, created_at FROM employees WHERE employee_code IS NULL OR employee_code = '' ORDER BY id ASC")->fetchAll(PDO::FETCH_ASSOC);
                        if (!empty($missingEmps)) {
                            foreach ($missingEmps as $emp) {
                                $dateStr = !empty($emp['joining_date']) ? $emp['joining_date'] : (!empty($emp['created_at']) ? $emp['created_at'] : date('Y-m-d'));
                                $time = strtotime($dateStr) ?: time();
                                $year = date('y', $time);
                                $month = date('m', $time);
                                $prefix = "SDS-{$year}-{$month}-";

                                $stmt = $pdo->prepare("SELECT employee_code FROM employees WHERE employee_code LIKE :prefix ORDER BY employee_code DESC LIMIT 1");
                    ...
                    // Clear demo attendance records before 2026-10-01
                    try {
                        $stmt = $pdo->query("SELECT setting_value FROM settings WHERE setting_key = 'demo_attendance_cleared_v1'");
                        $cleared = $stmt ? $stmt->fetchColumn() : null;
                        if ($cleared !== 'true') {
                            $pdo->exec("DELETE FROM employee_attendance WHERE date < '2026-10-01'");
                            $pdo->exec("INSERT INTO settings (setting_key, setting_value) VALUES ('demo_attendance_cleared_v1', 'true') ON DUPLICATE KEY UPDATE setting_value = 'true'");
                        }
                    } catch (\Exception $e) {}
                    ...
                    // Check and create company_expenses table
                    ...
                    // Check and create fuel_expenses table
