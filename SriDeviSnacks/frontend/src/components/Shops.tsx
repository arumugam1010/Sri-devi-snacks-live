  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
            ...
            Object.entries(schedulesResponse.data).forEach(([dayKey, daySchedules]: [string, any]) => {
              if (Array.isArray(daySchedules)) {
                ...
                daySchedules.forEach((schedule: any) => {
                  ...
                  } else if (schedule.shop && scheduleMap[day]) {
                    scheduleMap[day].push({
                      id: schedule.shop.id,
                      shop_name: schedule.shop.shopName || schedule.shop.shop_name,
                      address: schedule.shop.address,
                      contact: schedule.shop.contact,
                      email: schedule.shop.email,
                      gst: schedule.shop.gstNumber || schedule.shop.gst,
                      status: schedule.shop.status?.toLowerCase() === 'active' ? 'active' : 'inactive',
                      created_date: schedule.shop.createdAt ? new Date(schedule.shop.createdAt).toISOString().split('T')[0] : (schedule.shop.created_date || ''),
                    });
                  }
                });
              }
        ...
        setLoading(false);
        if (activeTab === 'schedule') setScheduleLoading(false);
      }
    };
              ...
              {daysOfWeek.map(day => {
                ...
                return (
                  <div 
                    ...
                    onClick={() => navigate(`/day-schedule/${day.toLowerCase()}`)}
                  >
                    ...
                    <div className="flex flex-col items-center justify-center py-8 text-center">
                      <Store className={`h-12 w-12 mb-4 ${dayShops.length === 0 ? 'text-gray-300' : 'text-blue-600'}`} />
                      <div className="text-3xl font-bold text-gray-900 mb-2">{dayShops.length}</div>
                      <div className="text-sm text-gray-600">
                        shop{dayShops.length !== 1 ? 's' : ''} assigned
                      </div>
                      {dayShops.length === 0 && (
                        <p className="text-gray-400 mt-2">No shops assigned to {day}</p>
                      )}
                    </div>
                  </div>
                );
              })}
                </div> {/* End Schedule Cards */}
