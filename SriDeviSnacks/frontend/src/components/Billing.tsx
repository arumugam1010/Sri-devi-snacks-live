  ...
  const Billing: React.FC = () => {
    ...
    const toggleGSTFiled = async (monthYear: string, currentStatus: boolean) => {
      try {
        setLoading(true);
        const res = await gstFilingsAPI.toggleFiling(monthYear, !currentStatus);
        if (res && res.success) {
          if (!currentStatus) {
            setGstFilings([...gstFilings, { month_year: monthYear, is_filed: 1 }]);
          } else {
            setGstFilings(gstFilings.filter(f => f.month_year !== monthYear));
          }
        }
      } catch (e: any) {
        console.error(e);
          ...
          <head>
            ...
            <style>
              * {
              ...
              .single-bill {
                width: 100%;
                padding: 40px; /* Added margin for PDF */
    ...
    try {
      ...
      if (doc) {
        ...
        for (let i = 0; i < elements.length; i++) {
          setExportProgress(`Processing bill ${i + 1} of ${elements.length}...`);
          if (i > 0) pdf.addPage();
          const canvas = await html2canvas(elements[i] as HTMLElement, { 
            scale: 2, // Increased scale slightly for better resolution
            useCORS: true,
            logging: false, // Turn off logging to prevent console spam
            allowTaint: true // Allow tainted canvas for base64 images just in case
          });
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex space-x-3">
            ...
            {/* Create Bill Button */}
            {!showBillingInterface && userRole !== 'ACCOUNTS' && (
              <div className="flex space-x-3">
                <button
                  onClick={() => {
          ...
          <div className="space-y-6">
            ...
            {(() => {
              ...
              if (selectedMonth && monthPaginationData) {
                ...
                const monthYearStr = `${monthName} ${financialYear}`;
                const isGstFiled = gstFilings.some(f => f.month_year === monthYearStr);

                return (
                  <div className={`rounded-lg shadow-sm border border-gray-200 p-6 ${isGstFiled ? 'bg-yellow-100' : 'bg-white'}`}>
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-4 pb-4 border-b border-gray-100">
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="flex space-x-2">
                          {(userRole === 'SUPER_ADMIN' || userRole === 'ACCOUNTS') && (
                            <>
                              {!isGstFiled && userRole === 'ACCOUNTS' && (
                                <button
                                  onClick={() => toggleGSTFiled(monthYearStr, isGstFiled)}
                                  disabled={loading}
                                  className="inline-flex items-center px-3 py-1.5 border text-xs font-semibold rounded-lg transition border-gray-300 text-gray-700 bg-white hover:bg-gray-50"
                                >
                                  {loading ? <Loader className="animate-spin h-3.5 w-3.5 mr-1" /> : null}
                                  Mark GST Filed
                                </button>
                              )}
                              ...
                              {sortedMonthBills.length > 0 && (
                                ...
                                <button
                                  onClick={() => handleExportPDF(sortedMonthBills.map(b => b.id), monthYearStr)}
                                  disabled={isExportingPDF}
                                  className={`inline-flex items-center px-4 py-2 border border-transparent text-xs font-bold rounded-lg text-white ml-2 shadow transition-all duration-200 ease-in-out transform hover:-translate-y-0.5 ${isExportingPDF ? 'bg-indigo-400 cursor-not-allowed' : 'bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 border-indigo-600/20 hover:shadow-md'}`}
                                >
                        ...
                        <div key={bill.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200 flex items-start">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex justify-between space-x-3">
                              ...
                              {bill.status !== 'COMPLETED' && userRole !== 'ACCOUNTS' && (
                                <button
                                  onClick={() => {
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {Object.entries(groupedBills[financialYear])
                          ...
                          .map(([monthName, monthBills]) => {
                            const mStr = `${monthName} ${financialYear}`;
                            const isFiled = gstFilings.some(f => f.month_year === mStr);
                            
                            return (
                            <div
                              key={monthName}
                              className={`rounded-lg p-4 cursor-pointer hover:opacity-90 transition-colors ${isFiled ? 'bg-yellow-100 border border-yellow-300' : 'bg-gray-50'}`}
